...
public class deleteElection extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("DELETE FROM elections WHERE election_id=?");
            preparedStatement.setString(1, request.getParameter("id"));
            
            if(preparedStatement.executeUpdate() > 0) {
                response.sendRedirect("admin/manage-elections#election-table");
            }
